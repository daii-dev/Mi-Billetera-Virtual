...
declare
  ...
  v_cuenta_referencia_id uuid;
  v_cuentas_usadas text;
begin
  ...
  where ama.meta_id = p_meta_id
    ...
    and a.clerk_user_id = v_usuario_clerk_id;

  v_cuenta_referencia_id := coalesce(v_meta.cuenta_id, p_cuenta_id);

  if v_cuenta_referencia_id is null then
    select ama.cuenta_id
    into v_cuenta_referencia_id
    from public.abonos_metas_ahorro ama
    where ama.meta_id = p_meta_id
      and ama.usuario_clerk_id = v_usuario_clerk_id
    group by ama.cuenta_id
    order by sum(ama.monto) desc, min(ama.creado_en) asc
    limit 1;
  end if;

  if v_cuenta_referencia_id is null then
    raise exception 'No se encontró una cuenta de referencia para la meta';
  end if;

  select *
  into v_cuenta
  from public.accounts
  where id = v_cuenta_referencia_id
    and clerk_user_id = v_usuario_clerk_id;

  if not found then
    raise exception 'La cuenta de referencia no existe o no pertenece al usuario';
  end if;
  ...
  values (
    ...
    concat_ws(
      ...
      'Meta: ' || v_meta.nombre,
      case
        when v_cuentas_usadas is not null then 'Cuentas usadas: ' || v_cuentas_usadas
        else null
      end
    ),
    current_date,
